...
// Course-scoped AI tutor — unlike the generic AiAssistantPanel (site-wide
// knowledge base), this hits /api/learning/ai-tutor/chat which grounds
// answers in this specific course's content and checks enrollment.
export function AiTutorPanel({ courseId, courseTitle }: { courseId: string; courseTitle: string }) {
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: "model", text: `მოგესალმებით! მე ვარ AI ინსტრუქტორი კურსისთვის „${courseTitle}“ — რით შემიძლია დაგეხმაროთ?` },
  ]);
  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/learning/ai-tutor/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ courseId, message: text, history }),
      });
    ...
    <div className="flex h-[26rem] flex-col">
      ...
      <form onSubmit={sendMessage} className="mt-4 flex items-center gap-2 border-t border-hairline pt-4">
        <input
          ...
          placeholder="დასვით კითხვა კურსის შესახებ…"
